    ...
    <div class="main-wrapper">
        ...
        <div class="ps-content">
            ...
            <div class="ps-card" id="card-periods">
                ...
                <div class="ps-card-body" id="card-periods-body">
...
<div id="createPeriodsModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);
     z-index:1000;display:none;align-items:center;justify-content:center;padding:20px;">
  <div style="background:#fff;border-radius:16px;padding:24px;width:480px;max-width:100%;
              box-shadow:0 20px 60px rgba(0,0,0,0.15);">

    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
      <h3 style="font-size:16px;font-weight:700;margin:0;">Create Pay Periods</h3>
      <button onclick="closeCreatePeriodsModal()"
              style="background:none;border:none;font-size:20px;cursor:pointer;color:#9ca3af;">&times;</button>
    </div>

    <p style="font-size:13px;color:#6b7280;margin:0 0 16px;">
      Periods are created using your current cutoff configuration
      (Days <?= $settings['cutoff1_start_day'] ?? 1 ?>–<?= $settings['cutoff1_end_day'] ?? 15 ?> and
      <?= $settings['cutoff2_start_day'] ?? 16 ?>–<?= $settings['cutoff2_end_day'] ?? 31 ?> of each month).
    </p>

    <div style="margin-bottom:14px;">
      <label style="font-size:12px;font-weight:600;display:block;margin-bottom:6px;">Create For</label>
      <div style="display:flex;gap:8px;">
        <label style="flex:1;cursor:pointer;">
          <input type="radio" name="ppCreateFor" value="month" checked
                 onchange="document.getElementById('ppMonthRow').style.display='flex'">
          <span style="display:block;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px;
                       text-align:center;font-size:13px;transition:border-color .15s;">
            <strong>Single Month</strong><br><small style="color:#9ca3af;">2 periods</small>
          </span>
        </label>
        <label style="flex:1;cursor:pointer;">
          <input type="radio" name="ppCreateFor" value="year"
                 onchange="document.getElementById('ppMonthRow').style.display='none'">
          <span style="display:block;border:1.5px solid #e5e7eb;border-radius:8px;padding:10px;
                       text-align:center;font-size:13px;transition:border-color .15s;">
            <strong>Full Year</strong><br><small style="color:#9ca3af;">24 periods</small>
          </span>
        </label>
      </div>
    </div>

    <div style="display:flex;gap:10px;margin-bottom:14px;" id="ppMonthRow">
      <div style="flex:1;">
        <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">Month</label>
        <select id="ppMonth" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;">
          <?php
          $monthNames2 = ['January','February','March','April','May','June',
                          'July','August','September','October','November','December'];
          $currentMonth = (int)date('n');
          foreach ($monthNames2 as $i => $mn):
          ?>
      ...
      <div style="flex:1;">
        ...
        <select id="ppYear" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;">
    ...
    <!-- Year-only select (hidden by default) -->
    <div style="display:none;margin-bottom:14px;" id="ppYearOnlyRow">
      <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">Year</label>
      <select id="ppYearOnly" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;">
        <?php for ($y = (int)date('Y'); $y <= (int)date('Y') + 3; $y++): ?>
        <option value="<?= $y ?>" <?= $y==(int)date('Y')?'selected':'' ?>><?= $y ?></option>
        <?php endfor; ?>
      </select>
    ...
    <div id="ppModalFlash" style="display:none;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:12px;"></div>

    <div style="display:flex;justify-content:flex-end;gap:10px;">
      <button onclick="closeCreatePeriodsModal()"
              style="padding:9px 16px;border:1px solid #e5e7eb;border-radius:8px;
                     background:#fff;cursor:pointer;font-size:13px;">
        Cancel
      </button>
      <button id="ppCreateBtn" onclick="submitCreatePeriods()"
              style="padding:9px 18px;background:#0f766e;color:#fff;border:none;
...
<script>
// ── Pay Period Manager ─────────────────────────────────────────────────────
const BASE_URL_PS = '<?= BASE_URL ?>';

function openCreatePeriodsModal() {
    document.getElementById('createPeriodsModal').style.display = 'flex';
    document.getElementById('ppModalFlash').style.display = 'none';
}
function closeCreatePeriodsModal() {
    document.getElementById('createPeriodsModal').style.display = 'none';
}

// Toggle month/year rows based on radio
document.querySelectorAll('input[name="ppCreateFor"]').forEach(r => {
    r.addEventListener('change', function () {
        ...
        document.querySelectorAll('input[name="ppCreateFor"]').forEach(rb => {
            rb.nextElementSibling.style.borderColor = rb.checked ? '#0f766e' : '#e5e7eb';
            rb.nextElementSibling.style.background  = rb.checked ? '#f0fdf4' : '';
        });
    });
});
...
async function submitCreatePeriods() {
    ...
    try {
        const res  = await fetch(`${BASE_URL_PS}actions/create-payroll-periods.php`, { method:'POST', body:fd });
        const data = await res.json();

        flash.style.display = 'block';
        flash.style.cssText = `display:block;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:12px;
            ${data.success
        ...
        flash.style.cssText = 'display:block;padding:10px;border-radius:8px;font-size:13px;background:#fee2e2;color:#991b1b;';
        btn.disabled  = false;
        btn.innerHTML = '<i class="bi bi-plus-lg"></i> Create';
    }
}
